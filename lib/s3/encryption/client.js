 ...
 */
 ...
 AWS.S3.Encryption.Client = AWS.util.inherit({
  ...
  getObject: function( params, callback ) {
    ...
    var self = this;
    // new instance of decrypter to support simultaneous requests (make optional?)
    var DecryptHandler = AWS.util.update({}, AWS.S3.Encryption.DecryptHandler); //AWS.S3.Encryption.DecryptHandler;
    ...
    DecryptHandler.on('decrypted', function( decrypt_body ) {
      ...
      // FIX: run callback only once - use .off method instead?
      callback = function(){};
    });
    // process request
